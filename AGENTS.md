# Staff CRM — context for Codex

Этот файл — компактный онбординг для следующей сессии Codex. Читать
...
- В конце пишем `Co-Authored-By: Codex Opus 4.7 <noreply@anthropic.com>`.
...
- Применяются вручную через Supabase Dashboard → SQL Editor (для прод —
  обязательно перед мержем PR; Codex напоминает пользователю).
...
SyncQueue эволюционировал и т.п.) — попроси меня обновить `AGENTS.md`
**первым делом**, потом продолжим.

## Находки при использовании портала

По просьбе пользователя от 2026-09-11 вести `docs/backlog/pilot-findings.md`: записывать обнаруженные проблемы, объединять повторы, отделять подтверждённые дефекты от предложений и вопросов на решение. Перед улучшениями UX читать этот журнал. Фиксация находки не означает, что предложенный способ исправления уже согласован. Не создавать из журнала реальные задачи Management OS.
